...
Many thanks to Mark Puttick that inspired the idea of the package, Andrew Beckerman, Natalie Cooper and Gavin Thomas for support during the development stage and Ian Brennan, Gustavo Burin, Christopher Cooney, Alex Cranston, William Geraty, Jasmine Hardie, Tom Lansley, Clement Prieul, Tiago Quental, Joe Rees, Louie Rombaut, James Rule and Sophie Ryan for comments and suggestions on late development stages.
...
OK, but what's the point of doing all these simulations?
As suggested before, there are probably many uses for simulating some evolutionary scenarios.
For example, you could use it to generate a series of null models to test some biological questions.
 ...
 * The extinction is random
 * The extinction targets specific trait values
...
So here the objective is to simulate a similar tree and traits and then simulate the extinction events.
Note that because of the stochastic nature of these simulations we will simulate a series of trees and traits, not just one.
This can easily be done in `treats` using the `replicates` option.
Here we will do 50 replicates and focus on just 2 traits.
...
## Simulate 50 trees and traits
set.seed(1)
sim_trees <- treats(bd.params  = my_bd_params,
                    stop.rule  = stop_rule,
                    trait      = my_traits,
                    null.error = 100,
                    replicates = 50,
...
plot(ladderize(sim_trees[[1]]$tree), show.tip.label = FALSE, main = "one simulated tree")
## Adding the phylogenetic axis
axisPhylo()

## Plotting one trait through time
plot(sim_trees[[1]], ylab = "Trait 1", las = 1, main = "Trait 1 through time")

## Plotting the two dimensions against each other
plot(sim_trees[[1]], trait = c(2,1), ylab = "Trait 1", xlab = "Trait 2", las = 1, main = "Traits 1 and 2")
```

## Creating extinction events

OK, now we can use the `make.events` function to design our extinction events.
This function works similarly to `make.modifiers` or `make.traits` and creates a `"treats"` and `"events"` object that can be fed to the `treats` function.
To create an event, we need to specify the following aspects:
...
```{r}
## Making a function that triggers and event 66 time units in the past
age.condition(x = 140-66, condition = `>=`)
...
`>=`(time, x)
```
 * *What the event is going to modify?* This should be also a function that, once the event is triggered, modifies the target of the simulation (e.g. modify the `bd.params` or the `traits` object). In our case, we will use the pre-implemented functions called `random.extinction` and `trait.extinction` that generate functions that modify the internal `"lineage"` object:

```{r}
## Making a function that removes 75% of species 
random.extinction(0.75)
...
And then simulate a bunch of trees and traits with these extinction events:
