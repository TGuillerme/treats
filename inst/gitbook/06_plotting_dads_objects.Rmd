...
plot(my_tree, main = "A tree and traits", col = c(tips = "pink", nodes = "purple"), pch = 21, lwd = 2, lty = 3, edges = "yellow")
```

> Note that you can modify colours using the `col` argument by providing either a clear indication of what you want to colour (e.g. `c(tip = "blue", `) @@@
...
## Plotting the 
plot(my_data, trait = c(1, 3), col =terrain.colors, tips.nodes = "black", legend = TRUE)
```
