...
This manual explains how \texttt{dads} work in as much details as possible and aims to give you the keys to design your own diversity and disparity simulations.
...
\NormalTok{devtools}\OperatorTok{::}\KeywordTok{install\_github}\NormalTok{(}\StringTok{"TGuillerme/dads"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The \texttt{dads} algorithm is based on a modular version of the birth-death process algorithm.
...
The algorithm used in \texttt{dads} allows modularity of this process and is based on the following steps (the text in \texttt{courier\ font} is for the name of the process in the algorithm):
...
\item
  \textbf{Simulating traits} {[}optional{]}(\texttt{traits}): this step allows to simulate a trait value for the selected lineage from step 1. This is typically not part of a standard birth-death process and is handled via the \texttt{traits} option in \texttt{dads} (see the \protect\hyperlink{maketraits}{\texttt{make.traits}} chapter for more details).
  Then go to step 4.
\item
...
Again, in \texttt{dads}, this process can be modified based on the birth-death parameters, the currently available lineages and potential trait values. \emph{For example, the lineage can only speciate if it's trait value is positive, regardless of whether speciation or extinction has been triggered}.
Then go to step 5.
...
\NormalTok{my\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ my\_stop\_rule)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{my\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ my\_params, }\DataTypeTok{stop.rule =}\NormalTok{ my\_stop\_rule)}
...
\NormalTok{my\_data \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ my\_params,}
...
\CommentTok{\#\# Simulate disparity and diversity}
\NormalTok{default\_data \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ extinction\_}\DecValTok{02}\NormalTok{,}
...
This function intakes the \texttt{trait.values} and \texttt{parent.lineage} arguments, two arguments that you can leave named as they are to facilitate \texttt{dads}' understanding of what you want to assess:
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\CommentTok{\#\# Simulate disparity and diversity}
\NormalTok{biased\_data \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ extinction\_}\DecValTok{02}\NormalTok{,}
                    \DataTypeTok{stop.rule =}\NormalTok{ living\_}\DecValTok{20}\NormalTok{,}
                    \DataTypeTok{traits    =}\NormalTok{ BM\_trait,}
...
In \texttt{dads}, traits are simulating by providing a \texttt{traits} object to the \texttt{traits} argument.
...
For the purpose of the syntax in \emph{this package} though, a \textbf{trait in \texttt{dads}} is a column in the output trait matrix.
...
This single ``beak shape'' \textbf{biological trait} is then composed of 912 \textbf{traits in \texttt{dads}} (i.e.~a matrix of \((4+300)\times3\) columns).
\end{quote}
...
\begin{quote}
Note that in this function definition the arguments \texttt{x0} and \texttt{edge.length} have a default value set to \texttt{0} and \texttt{1} respectively. In practice, these arguments are effectively set to the correct values in the \texttt{dads} internal function (i.e.~whatever \texttt{x0} and \texttt{edge.length} are at that specific time of the process) but providing a default can help speed up the algorithms (specifically all the internal checks).
\end{quote}
...
\hypertarget{using-a-process-in-dads}{%
\subsection{\texorpdfstring{Using a \texttt{"process"} in \texttt{dads}}{Using a "process" in dads}}\label{using-a-process-in-dads}}

You can design your own process as a function (as long as it has a valid syntax).
Alternatively, the \texttt{dads} package proposes inbuilt processes, namely a multidimensional Brownian motion (\texttt{BM.process}) or a a multidimensional Ornstein-Uhlenbeck process (\texttt{OU.process}).
...
Note that you can see the multiple options for plotting the trait process by looking at \texttt{?plot.dads} manual. Furthermore, you can look at what's actually in the object using this specific syntax (this applies to every object handled by the \texttt{dads} package):

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#\# What\textquotesingle{}s actually in that object?}
\KeywordTok{print.dads}\NormalTok{(my\_trait\_object, }\DataTypeTok{all =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Because it would be impossible to accommodate all definitions in \texttt{dads} we had to go with an arbitrary one: a trait is whatever you define as a trait!
...
In \texttt{dads}, the \textbf{process} corresponds to this trait definition (e.g.~a process can be of n-dimensions and represents one organisms feature) and the \textbf{traits} represents the number of dimensions in total.
So in the examples above, this is how the following traits are interpreted by \texttt{dads}:
...
\NormalTok{tree\_bkg \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{max.taxa =} \DecValTok{20}\NormalTok{),}
                 \DataTypeTok{traits    =}\NormalTok{ my\_trait)}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-41-1.pdf}
...
Alternatively to having a background trait, you can also simulate a tree by generating traits at specific time steps with the \texttt{save.steps} option in \texttt{dads}.
...
\NormalTok{tree\_steps \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule  =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{max.time =} \DecValTok{3}\NormalTok{),}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-42-1.pdf}

\hypertarget{traits-implemented-in-dads}{%
\section{\texorpdfstring{Traits implemented in \texttt{dads}}{Traits implemented in dads}}\label{traits-implemented-in-dads}}

If you don't want to design your own trait process, you can use one of the following trait processes that are currently implemented in \texttt{dads}. You can find more information about their many options using their specific manuals in R or the generic \texttt{?trait.process}:
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-43-1.pdf}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-46-1.pdf}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-47-1.pdf}
...
\texttt{make.traits} will send an error if the trait cannot be directly passed to \texttt{dads}.
...
Remember it is good practice for \texttt{dads} processes to set all the arguments with default values (just in case).
...
The modifiers modify the core of the birth death process as implemented in \texttt{dads}.

By default, the birth-death process in \texttt{dads} uses this modifier:
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{runif}\NormalTok{(}\DecValTok{1}\NormalTok{) }\OperatorTok{\textless{}}\StringTok{ }\NormalTok{(bd.params}\OperatorTok{$}\NormalTok{speciation}\OperatorTok{/}\NormalTok{(bd.params}\OperatorTok{$}\NormalTok{speciation }\OperatorTok{+}\StringTok{ }\NormalTok{bd.params}\OperatorTok{$}\NormalTok{extinction))}\ErrorTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{default\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd\_params,}
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{0}\NormalTok{)}
\NormalTok{modified\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd\_params,}
                      \DataTypeTok{stop.rule =}\NormalTok{ stop\_rule,}
...
\CommentTok{\#\# Generating a new tree with this modifier}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{0}\NormalTok{)}
\NormalTok{increasing\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd\_params,}
                        \DataTypeTok{stop.rule =}\NormalTok{ stop\_rule,}
...
\CommentTok{\#\# Generating a new tree with this modifier}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{0}\NormalTok{)}
\NormalTok{ladderised\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd\_params,}
                        \DataTypeTok{stop.rule =}\NormalTok{ stop\_rule,}
...
For example, we can create a \texttt{"dads"} object that also generates a BM trait and add to it this object a \texttt{selection} modifier that only selects tips with positive trait values (only species with positive trait values will speciate).
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{default\_dads \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd\_params,}
                     \DataTypeTok{stop.rule =}\NormalTok{ stop\_rule,}
                     \DataTypeTok{traits    =}\NormalTok{ BM\_trait)}

\CommentTok{\#\# Simulate a tree and trait with the modifier}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{skewed\_trait\_dads \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd\_params,}
                          \DataTypeTok{stop.rule =}\NormalTok{ stop\_rule,}
                          \DataTypeTok{traits    =}\NormalTok{ BM\_trait,}
...
\KeywordTok{par}\NormalTok{(}\DataTypeTok{mfrow =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{))}
\KeywordTok{plot}\NormalTok{(default\_dads, }\DataTypeTok{main =} \StringTok{"Default trait and tree"}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(skewed\_trait\_dads, }\DataTypeTok{main =} \StringTok{"Skewed trait and tree"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\#\# Simulate a tree and trait with the modifier}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{biased\_trait\_dads \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd\_params,}
                          \DataTypeTok{stop.rule =}\NormalTok{ stop\_rule,}
                          \DataTypeTok{traits    =}\NormalTok{ BM\_trait,}
...
\KeywordTok{par}\NormalTok{(}\DataTypeTok{mfrow =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{))}
\KeywordTok{plot}\NormalTok{(default\_dads, }\DataTypeTok{main =} \StringTok{"Default trait and tree"}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(biased\_trait\_dads, }\DataTypeTok{main =} \StringTok{"Biased trait and tree"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-67-1.pdf}
...
\NormalTok{    trigger\_event \textless{}{-}}\StringTok{ }\KeywordTok{runif}\NormalTok{(}\DecValTok{1}\NormalTok{)}
    ...
    \CommentTok{\#\# Speciate?}
    \KeywordTok{return}\NormalTok{(trigger\_event }\OperatorTok{\textless{}}\StringTok{ }\NormalTok{(bd.params}\OperatorTok{$}\NormalTok{speciation}\OperatorTok{/}\NormalTok{(bd.params}\OperatorTok{$}\NormalTok{speciation }\OperatorTok{+}\StringTok{ }\NormalTok{bd.params}\OperatorTok{$}\NormalTok{extinction)))}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
One other major feature of the \texttt{dads} package is to allow simulations to run with specified events to occur during the simulation.
...
These are all handled by \texttt{dads} with the \texttt{events} object that you can create using \texttt{make.events} and has the same overall logic as \texttt{make.traits} and \texttt{make.modifiers}.
...
* \texttt{"founding"} this target is a bit more special and allows you run a nested \texttt{dads} object in the simulation. It is cover in a \protect\hyperlink{founding}{specific section below}.
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{)}
\NormalTok{results \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd.params,}
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{no\_change \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd.params,}
                  \DataTypeTok{stop.rule =}\NormalTok{ stop.rule,}
                  \DataTypeTok{traits    =}\NormalTok{ traits)}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{process\_change \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd.params,}
                       \DataTypeTok{stop.rule =}\NormalTok{ stop.rule,}
                       \DataTypeTok{traits    =}\NormalTok{ traits,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-76-1.pdf}
...
It basically allows you to simulate a specific \texttt{dads} process i.e.~using the \texttt{dads} function with it's own \texttt{traits}, \texttt{modifiers}, \texttt{bd.params} and \texttt{events} (and yes, that's including \texttt{events} that have their own \texttt{founding} event)\ldots{}
This is basically the ultimate nested boss of modularity!

The founding event will basically run an internal \texttt{dads} process (i.e.~simulating a tree and, optionally, some data) resulting in a founding sub-tree.
...
This function intakes the exact same arguments as \texttt{dads} to simulate the sub-tree with its own parameters.
...
\NormalTok{founding\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =}\NormalTok{ bd.params,}
                      \DataTypeTok{stop.rule =}\NormalTok{ stop.rule,}
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(}\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{max.taxa =} \DecValTok{50}\NormalTok{), }\DataTypeTok{bd.params =}\NormalTok{ my\_bd\_params))}
\end{Highlighting}
\end{Shaded}

\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-79-1.pdf}
...
In this example, the \texttt{"bd.params"} object passed to the \texttt{dads} function will allow the birth-death process to sample the speciation parameter each time it is called (e.g.~during the speciation/extinction step, the branch length step, etc.).
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-84-1.pdf}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-84-2.pdf}
...
\NormalTok{my\_data \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{max.taxa =} \DecValTok{20}\NormalTok{),}
...
\hypertarget{dads-internal-utilities}{%
\section{\texorpdfstring{\texttt{"dads"} internal utilities}{"dads" internal utilities}}\label{dads-internal-utilities}}
...
These functions don't do anything useful on their own but are optimised to be used internally in \texttt{dads}.
...
\texttt{"dads"} objects can be directly plotted in \texttt{dads} using the S3 \texttt{plot.dads} function (or just \texttt{plot(x)} if \texttt{x} is of class \texttt{"dads"}).

\hypertarget{plotting-traits}{%
\chapter{Plotting traits}\label{plotting-traits}}

\texttt{"dads"} \texttt{"traits"} objects are covered in the \protect\hyperlink{maketraits}{traits section}.
You can use \texttt{plot.dads} to plot them by choosing which specific trait to plot using the \texttt{trait} argument (default is 1):
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-87-1.pdf}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-88-1.pdf}

\hypertarget{plotting-dads-results}{%
\chapter{Plotting dads results}\label{plotting-dads-results}}

If \texttt{dads} is used to plot only a tree (and outputs a \texttt{"phylo"} object), you can use the function \texttt{plot.phylo} from the \texttt{ape} package to plot your tree.
You'll find all the tree plotting option in the \texttt{?plot.phylo} manual page.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#\# A simple pure birth tree}
\NormalTok{my\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{max.taxa =} \DecValTok{20}\NormalTok{))}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-89-1.pdf}

However, if you also simulated a trait along with the tree you can use the \texttt{plot.dads} function to plot both the tree and the trait:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#\# A simple pure birth tree with a BM process}
\NormalTok{my\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{max.taxa =} \DecValTok{20}\NormalTok{), }\DataTypeTok{traits =} \KeywordTok{make.traits}\NormalTok{())}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-90-1.pdf}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-91-1.pdf}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-92-1.pdf}
...
\NormalTok{my\_data \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{bd.params =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{extinction =} \FloatTok{0.2}\NormalTok{),}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-94-1.pdf}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-95-1.pdf}

But there's more! You can also plot these results using actual 3D plots that you can spin and all that!
This is done through the \texttt{rgl} package and activated in \texttt{dads} using the option \texttt{use.3D\ =\ TRUE}.
...
\includegraphics{../../../../../../tmp/Rtmp8BGjPN/file97bd69ab2a6.png}
...
\includegraphics{../../../../../../tmp/Rtmp8BGjPN/file97bd7e5353fb.png}
...
\CommentTok{\#\# Running the simulations}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{results \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{6}\NormalTok{,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-101-1.pdf}
...
\NormalTok{results \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{5}\NormalTok{,}
                \DataTypeTok{bd.params =}\NormalTok{ speciation\_}\DecValTok{1}\NormalTok{,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-105-1.pdf}
...
\CommentTok{\#\# Running the simulations}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{2}\NormalTok{)}
\NormalTok{results \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_taxa\_}\DecValTok{50}\NormalTok{,}
                \DataTypeTok{bd.params =}\NormalTok{ speciation\_}\DecValTok{1}\NormalTok{,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-108-1.pdf}
...
\NormalTok{no\_event \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{6}\NormalTok{,}
                 \DataTypeTok{bd.params =}\NormalTok{ random\_speciation)}

\CommentTok{\#\# Reduced speciation event}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{42}\NormalTok{)}
\NormalTok{reduced\_speciation\_event \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{6}\NormalTok{,}
                                 \DataTypeTok{bd.params =}\NormalTok{ random\_speciation,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-111-1.pdf}
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{no\_change \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{6}\NormalTok{,}
                  \DataTypeTok{bd.params =}\NormalTok{ speciation\_}\DecValTok{1}\NormalTok{,}
                  \DataTypeTok{traits    =}\NormalTok{ simple\_bm\_trait)}
\CommentTok{\#\# Run the simulations with change}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{process\_change \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{6}\NormalTok{,}
                       \DataTypeTok{bd.params =}\NormalTok{ speciation\_}\DecValTok{1}\NormalTok{,}
                       \DataTypeTok{traits    =}\NormalTok{ simple\_bm\_trait,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-115-1.pdf}
...
\CommentTok{\#\# Run the simulations}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{2}\NormalTok{)}
\NormalTok{no\_event \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_taxa\_}\DecValTok{100}\NormalTok{,}
                 \DataTypeTok{bd.params =}\NormalTok{ speciation\_}\DecValTok{1}\NormalTok{,}
                 \DataTypeTok{traits    =}\NormalTok{ correlated\_2D\_BM)}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{2}\NormalTok{)}
\NormalTok{change\_correlation \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_taxa\_}\DecValTok{100}\NormalTok{,}
                           \DataTypeTok{bd.params =}\NormalTok{ speciation\_}\DecValTok{1}\NormalTok{,}
                           \DataTypeTok{traits    =}\NormalTok{ correlated\_2D\_BM,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-119-1.pdf}
...
\includegraphics{../../../../../../tmp/Rtmp8BGjPN/file97bd12f0c3a.png}
...
\NormalTok{no\_event \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{4}\NormalTok{,}
                 \DataTypeTok{bd.params =}\NormalTok{ speciation\_}\DecValTok{1}\NormalTok{,}
                 \DataTypeTok{traits    =}\NormalTok{ simple\_bm\_trait,}
                 \DataTypeTok{modifiers =}\NormalTok{ default\_modifiers)}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{4}\NormalTok{)}
\NormalTok{change\_spec \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{4}\NormalTok{,}
                    \DataTypeTok{bd.params =}\NormalTok{ speciation\_}\DecValTok{1}\NormalTok{,}
                    \DataTypeTok{traits    =}\NormalTok{ simple\_bm\_trait,}
                    \DataTypeTok{modifiers =}\NormalTok{ default\_modifiers,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-125-1.pdf}

\hypertarget{EG_modify_brlen}{%
\chapter{Changing branch length when reaching n taxa}\label{EG_modify_brlen}}
...
\NormalTok{no\_event \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_taxa\_}\DecValTok{100}\NormalTok{,}
                 ...
                 \DataTypeTok{modifiers =}\NormalTok{ default\_modifiers)}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{5}\NormalTok{)}
\NormalTok{increased\_brlen \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_taxa\_}\DecValTok{100}\NormalTok{,}
                        \DataTypeTok{bd.params =}\NormalTok{ speciation\_}\DecValTok{1}\NormalTok{,}
                        \DataTypeTok{modifiers =}\NormalTok{ default\_modifiers,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-129-1.pdf}
...
\CommentTok{\#\# Simulations}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{11}\NormalTok{)}
\NormalTok{founding\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{4}\NormalTok{,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-132-1.pdf}
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{founding\_tree \textless{}{-}}\StringTok{ }\KeywordTok{dads}\NormalTok{(}\DataTypeTok{stop.rule =}\NormalTok{ stop\_time\_}\DecValTok{6}\NormalTok{,}
...
\includegraphics{dads_manual_files/figure-latex/unnamed-chunk-136-1.pdf}
