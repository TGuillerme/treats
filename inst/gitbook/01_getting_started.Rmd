...
In other words, if the lineages trait value is negative, make sure that _x_ is equal to 1 (so that the speciation condition $x < (\lambda \ (\lambda + \mu))$ is never met).
...
This function intakes the `trait.values` and `parent.lineage` arguments, two arguments that you can leave named as they are to facilitate `dads`' understanding of what you want to assess:
