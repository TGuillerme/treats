...
time_series <- chrono.subsets(data = BeckLee_mat99, tree = BeckLee_tree, method = "continuous", model = "proximity", time = seq(from = 101, to = 41, by = -5))
...
```{r}
## TODO: make this automatically do this:
# time_series <- chrono.subsets(data = dispRitreats(sim_data)$data, tree = dispRitreats(sim_data)$tree, method = "continuous", model = "proximity", time = 13)

## Converting the data to be passed to dispRity functions
dispRity_data <- dispRitreats(sim_data)

## Creating the time series
time_series <- chrono.subsets(data = dispRity_data$data, tree = dispRity_data$tree, time = seq(from = 0.9, to = 0, by = -0.1), method = "continuous", model = "proximity")

simulated_disparity <- dispRity(time_series, metric = c(sum, variances), verbose = TRUE)
