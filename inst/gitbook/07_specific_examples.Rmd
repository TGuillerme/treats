...
`update.bd.params` | [background extinction](#EGbg_ext), [reducing speciation rate](#EGred_spec)
`update.traits` | [changing trait process](#EG_change_trait), [changing trait correlation](#EG_change_correlation)
...
`update.modifiers` | [changing modifiers](#EG_change_modif), [change branch length](#EG_modify_brlen)
...
We will then create a event that triggers when reaching half of the simulation (using `time.condition(2.5)`).
...
## Species with negative trait values go extinct after some time {#EGneg_ext}
...
## Reducing speciation rate after some time {#EGred_spec}
...
## Simulation parameters
stop_time_6 <- list(max.time = 6)
random_speciation <- make.bd.params(speciation = runif, speciation.args = list(min = 0.5, max = 1))
```
...
## Founding event: a generating a subtree with no fossils {#EG_founding_purebirth}
...
## Founding event: a generating a subtree a different process {#EG_founding_traits}
