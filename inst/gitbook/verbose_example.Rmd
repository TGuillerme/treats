...
simulation <- dads(bd.params = my_params, stop.rule = my_stop_rule, trait = trait)
...
par(mfrow = c(2,1))
plot(dads(bd.params = bd.params, stop.rule = stop.rule, traits = traits), main = "no modif")
...
plot(dads(bd.params = bd.params, stop.rule = stop.rule, traits = traits, modifiers = modifiers), main = "Speciation increase\nwith absolute trait value")
