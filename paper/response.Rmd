...
I've changed the comment to `## Simulate the tree and traits with a selective extinction event`. line @@@
...
I've added references to figures 5 and 6 in the text on lines @@@ and @@@.
...
For more details on simulating the effect of mass extinction and the difficulties to simulate an unambiguous effect of a mass extinction, see @puttick2020complex."* lines @@@
...
I tried (now in tests/) but I'm not convinced how useful it is.
...
Using 30 time units here allows to simulate number of tips in a similar order of magnitude as the ones in the observed data."* lines @@@
...
> TODO: specify this in the manual and the documentation: `modification` = modify the bd-algorithm; `events` (conditions and stuff) = modify the flow of the algorithm (but can also introduce a `modification`!)
