...
4. Generating some trait(s) value(s) for the selected lineage (either a tip or a node).
...
```{r prepdata, echo = FALSE, fig.cap="Figure 2: Observed phylogeny (A) and disparity through time (B) in the subset of @beck2014ancient's dataset. The red line represents the K-Pg boundary. Is this change in disparity related to the K-Pg mass extinction? Or, more generally, is it even possible to detect potential changes of disparity due to a mass extinction event?", fig.width = 18, fig.height = 8}
...
These two parameters can be crudely calculated from a given tree by counting the number of speciations and extinctions through time (but see better methods, e.g. `ape::birthdeath`; @lawton1995extinction; @ape).
...
The second one will simulate a random extinction but based on trait values: after reaching time 15, all the species with positive trait values will go extinct.
Both scenarios thus illustrate two different types of mass extinction (random and selective; @puttick2020complex).
Because of the stochasticity of the simulations, we will repeat them 50 times (using `replicates = 50`) to generate a distribution of possible simulated scenarios as opposed to a random single one that could be idiosyncratic.
...
The `treats` package comes with internal documentation (e.g. `?treats`) but also with a thorough and extended vignette in a gitbook format: the [`treats` manual](http://tguillerme.github.io/treats.html) ([http://tguillerme.github.io/treats.html](http://tguillerme.github.io/treats.html).
