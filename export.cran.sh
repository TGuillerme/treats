...
rm -R cran_tmp/
mkdir cran_tmp
mkdir cran_tmp/treats
TMPPATH="cran_tmp/treats"
...
## Compile the package
cd cran_tmp/
R CMD build --resave-data treats

## Check the package
R CMD check --as-cran treats_*.tar.gz
#--install-args=_R_CHECK_LENGTH_1_LOGIC2_ 
#--as-cran 

## Check compile
if grep -e 'ERROR' -e 'WARNING' treats.Rcheck/00check.log 
then
    echo "You broke it again! Bordel!"
    cd ..
else

    if grep -e 'NOTE'  treats.Rcheck/00check.log 
    then
        echo "Check out the NOTE(s) before submitting!"
        echo "/Users/TGuillerme/Packaging/treats/00check.log"
        mv ~/Packaging/treats/cran_tmp/treats.Rcheck/00check.log ~/Packaging/treats/00check.log
    else
        echo "Nice one: it compiles smoothly!"
    fi    
    echo "The tar ball has been moved to ~/Packaging/CRAN/"
    mv treats_*.tar.gz ~/Packaging/CRAN/
    cd ..
    rm -R cran_tmp/
fi
